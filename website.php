...
<script>
    $(function() {
        ...
        $('.btn-del-group').click(function(){
            if(confirm("Czy napewno chcesz usunąć test " + $(this).attr('data-name') +"?")){
                $.ajax({
                    type: "POST",
                    url: "ajax.php",
                    data: {
...
<?php
    ...
    while($result && ($row=mysql_fetch_assoc($result))){
        echo "<div class='rows'><ul> \r\n" .
            ...
            "<li class='bton'> \r\n".
            " <a class='btn' href='edit.php?idGroup=".$row['idGroup']."#".h8($row['tname'])."' ><img src='img/setting.gif' rel='tooltip' title='Zmień ustawienia' /></a> \r\n".
            " <a class='btn group-btn-info' data-id='".$row['idTarget']."' ><img src='img/info.gif' rel='tooltip' title='Zmień opis' /></a> \r\n".
            " <a class='btn group-btn-calendar' data-id='".$row['idTarget']."'><img src='img/edit.gif' rel='tooltip' title='Przeglądaj otatki' /></a></li> \r\n".
            " <a class='btn btn-danger btn-del-group' data-id='".$row['idTarget']."' data-name='".str_replace('*','',h8($row['gname']))."' ><img src='img/trash.png' rel='tooltip' title='Usuń test - ".str_replace('*','',h8($row['gname']))."' /></a> \r\n".
